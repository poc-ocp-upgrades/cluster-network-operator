...
var OpenShiftSDNConfig = operv1.Network{Spec: operv1.NetworkSpec{ServiceNetwork: []string{"172.30.0.0/16"}, ClusterNetwork: []operv1.ClusterNetworkEntry{{CIDR: "10.128.0.0/15", HostPrefix: 23}, {CIDR: "10.0.0.0/14", HostPrefix: 24}}, DefaultNetwork: operv1.DefaultNetworkDefinition{Type: operv1.NetworkTypeOpenShiftSDN, OpenShiftSDNConfig: &operv1.OpenShiftSDNConfig{Mode: operv1.SDNModeNetworkPolicy}}}}
...
func TestFillOpenShiftSDNDefaults(t *testing.T) {
	...
	p := uint32(4789)
	m := uint32(8950)
	expected := operv1.NetworkSpec{ServiceNetwork: []string{"172.30.0.0/16"}, ClusterNetwork: []operv1.ClusterNetworkEntry{{CIDR: "10.128.0.0/15", HostPrefix: 23}, {CIDR: "10.0.0.0/14", HostPrefix: 24}}, DefaultNetwork: operv1.DefaultNetworkDefinition{Type: operv1.NetworkTypeOpenShiftSDN, OpenShiftSDNConfig: &operv1.OpenShiftSDNConfig{Mode: operv1.SDNModeNetworkPolicy, VXLANPort: &p, MTU: &m}}, DeployKubeProxy: &f, KubeProxyConfig: &operv1.ProxyConfig{BindAddress: "0.0.0.0", ProxyArguments: map[string][]string{"metrics-bind-address": {"0.0.0.0"}, "metrics-port": {"9101"}}}}
...
func TestValidateOpenShiftSDN(t *testing.T) {
	...
	errExpect := func(substr string) {
		t.Helper()
		g.Expect(validateOpenShiftSDN(config)).To(ContainElement(MatchError(ContainSubstring(substr))))
	}
	mtu := uint32(70000)
	sdnConfig.MTU = &mtu
...
func TestProxyArgs(t *testing.T) {
	...
	config.KubeProxyConfig = &operv1.ProxyConfig{IptablesSyncPeriod: "10s", BindAddress: "1.2.3.4"}
	objs, err = renderOpenShiftSDN(config, manifestDir)
	g.Expect(err).NotTo(HaveOccurred())
	cfg = getSdnConfigFile(objs)
	...
	config.KubeProxyConfig.ProxyArguments = map[string][]string{"a": {"b"}, "c": {"d", "e"}}
	objs, err = renderOpenShiftSDN(config, manifestDir)
	g.Expect(err).NotTo(HaveOccurred())
	cfg = getSdnConfigFile(objs)
...
func TestOpenShiftSDNMultitenant(t *testing.T) {
	...
	objs, err := renderOpenShiftSDN(config, manifestDir)
	g.Expect(err).NotTo(HaveOccurred())
	netNS := []string{"openshift-dns", "openshift-ingress", "openshift-monitoring", "openshift-kube-apiserver", "openshift-apiserver", "kube-system", "openshift-operator-lifecycle-manager", "openshift-image-registry"}
