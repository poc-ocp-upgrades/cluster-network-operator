...
func fillOpenShiftSDNDefaults(conf, previous *operv1.NetworkSpec, hostMTU int) {
	...
	if sc.MTU == nil {
		var mtu uint32 = uint32(hostMTU) - 50
		if previous != nil && previous.DefaultNetwork.Type == operv1.NetworkTypeOpenShiftSDN && previous.DefaultNetwork.OpenShiftSDNConfig != nil && previous.DefaultNetwork.OpenShiftSDNConfig.MTU != nil {
...
func controllerConfig(conf *operv1.NetworkSpec) (string, error) {
	...
	cfg := cpv1.OpenShiftControllerManagerConfig{TypeMeta: metav1.TypeMeta{APIVersion: "openshiftcontrolplane.config.openshift.io/v1", Kind: "OpenShiftControllerManagerConfig"}, Network: cpv1.NetworkControllerConfig{NetworkPluginName: sdnPluginName(c.Mode), ClusterNetworks: ippools, ServiceNetworkCIDR: conf.ServiceNetwork[0], VXLANPort: *c.VXLANPort}}
...
func nodeConfig(conf *operv1.NetworkSpec) (string, error) {
	...
	defer _logClusterCodePath()
	c := conf.DefaultNetwork.OpenShiftSDNConfig
	result := legacyconfigv1.NodeConfig{TypeMeta: metav1.TypeMeta{APIVersion: "v1", Kind: "NodeConfig"}, NetworkConfig: legacyconfigv1.NodeNetworkConfig{NetworkPluginName: sdnPluginName(c.Mode), MTU: *c.MTU}, ServingInfo: legacyconfigv1.ServingInfo{ClientCA: "/var/run/secrets/kubernetes.io/serviceaccount/ca.crt", BindAddress: conf.KubeProxyConfig.BindAddress + ":10251"}, KubeletArguments: legacyconfigv1.ExtendedArguments{"container-runtime": {"remote"}, "container-runtime-endpoint": {"unix:///var/run/crio/crio.sock"}}, IPTablesSyncPeriod: conf.KubeProxyConfig.IptablesSyncPeriod, ProxyArguments: conf.KubeProxyConfig.ProxyArguments}
